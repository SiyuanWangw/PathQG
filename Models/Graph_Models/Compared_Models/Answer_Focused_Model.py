...
class Generator(nn.Module):
    def __init__(self, args, pad_idx=0, pretrain_src_embeddings=None, pretrain_tgt_embeddings=None):
        ...
        if not self.args.attn_input_feed:
            ...
            self.decoder_lstm_cell = nn.LSTMCell(self.args.embedding_size + self.args.decoder_hidden_size,
                                                 self.args.decoder_hidden_size)
        self.sentence_attn = BahdanauAttention(self.args.decoder_hidden_size, self.args.encoder_hidden_size*2+self.args.tagging_embedding_size, self.args.attn_size)
        self.dropout = nn.Dropout(self.args.dropout)

        self.dense = nn.Linear(self.args.decoder_hidden_size+self.args.encoder_hidden_size*2+self.args.tagging_embedding_size, self.args.question_dense_size)
        self.out = nn.Linear(self.args.question_dense_size, self.args.tgt_vocab_size)
        self.wh_dense = nn.Linear(self.args.decoder_hidden_size+self.args.encoder_hidden_size*2+self.args.embedding_size+self.args.tagging_embedding_size, self.args.question_dense_size)
        self.wh_out = nn.Linear(self.args.question_dense_size, 9)
        self.p_gen_dense = nn.Linear(self.args.decoder_hidden_size + self.args.encoder_hidden_size * 4 + self.args.embedding_size+self.args.tagging_embedding_size, 2)
        torch.nn.init.xavier_uniform_(self.dense.weight)
        torch.nn.init.xavier_uniform_(self.out.weight)
        torch.nn.init.xavier_uniform_(self.wh_dense.weight)
        torch.nn.init.xavier_uniform_(self.wh_out.weight)
        torch.nn.init.xavier_uniform_(self.p_gen_dense.weight)
        self.is_test = False

        self.index = torch.tensor([[5, 20, 19, 30, 14, 214, 229, 56, 13],]*self.args.batch_size).long().cuda()
    ...
    def init_embedding(self, pad_idx, pretrain_src_embeddings, pretrain_tgt_embeddings):
        ...
        self.args.distance_tag_num = 150
        # self.args.distance_tag_num = 200
        self.distance_tagging_embeddings = nn.Embedding(self.args.distance_tag_num, self.args.tagging_embedding_size)
        self.distance_tagging_embeddings.weight.data.uniform_(-0.1, 0.1)

    def encode(self, src_input, answer_tag_input, graph_tag_input, src_input_lengths):
    ...
    def decode(self, tgt_input, init_hidden, encoder_outputs, src_input, answer_info):
        ...
        for t in range(tgt_input.size(1)):
            ...
            p_gen = F.softmax(self.p_gen_dense(torch.cat([cur_context, h_t, input_embed], -1)), -1)

            tanh_fc = F.relu(self.dense(concat_output.squeeze(1)))
            p_vocab = F.softmax(self.out(tanh_fc), -1)

            wh_tanh_fc = F.relu(self.wh_dense(torch.cat([answer_info.unsqueeze(1), concat_output], -1).squeeze(1)))
            p_wh_vocab = F.softmax(self.wh_out(wh_tanh_fc), -1)

            p_copy = torch.zeros([p_vocab.size(0), self.args.tgt_vocab_size]).cuda()
            p_copy = p_copy.scatter_(1, self.index, p_wh_vocab)
            logit = p_gen[:, 0].unsqueeze(1) * p_vocab + p_gen[:, 1].unsqueeze(1) * p_copy
        ...
        logits = torch.log(torch.cat(logits, 1)+1e-10)
        return logits
    ...
    def test_decoder(self, cur_input, init_hidden, t):
        if cur_input.size(0) == self.args.batch_size:
            ...
            answer_info = torch.cat([self.answer_info.unsqueeze(1),]*self.args.beam_size, 1).view(-1, self.answer_info.size(1))
            index = torch.cat([self.index.unsqueeze(1),]*self.args.beam_size, 1).view(-1, self.index.size(1))
        ...
        p_copy = torch.zeros([p_vocab.size(0), self.args.tgt_vocab_size]).cuda()
        p_copy = p_copy.scatter_(1, index, p_wh_vocab)
        logit = p_gen[:, 0].unsqueeze(1) * p_vocab + p_gen[:, 1].unsqueeze(1) * p_copy
        # logit = logit.unsqueeze(1)
    ...
    def forward(self, src_input, answer_tag_input, graph_tag_input, distance_tag_input, src_input_lengths, node_input, edge_input, adjency_matrix, answer_input, tgt_input):
        encode_outputs, init_state = self.encode(src_input, answer_tag_input, graph_tag_input, src_input_lengths)
        distance_tag_input_embed = self.distance_tagging_embeddings(distance_tag_input)

        encode_outputs = torch.cat([encode_outputs, distance_tag_input_embed], -1)

        mask = torch.sign(answer_input).float().unsqueeze(-1)
        answer_embedding = self.src_embedding(answer_input)
        average_answer_embeddings = torch.sum(mask.mul(answer_embedding), 1) / torch.sum(mask, 1).clamp(1)

        if not self.is_test:
            logits = self.decode(tgt_input, init_state, encode_outputs, src_input, average_answer_embeddings)
            return logits
        else:
            # beam search
            if self.args.beam_size > 1:
                self.answer_info = average_answer_embeddings
