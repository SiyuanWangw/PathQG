...
class Generator(nn.Module):
    def __init__(self, args, pad_idx=0, pretrain_src_embeddings=None, pretrain_tgt_embeddings=None):
        ...
        self.encoder_lstm = nn.LSTM(self.args.embedding_size + self.args.tagging_embedding_size,
                                    self.args.encoder_hidden_size,
                                    num_layers=self.args.num_layers,
                                    bidirectional=True,
                                    dropout=self.args.dropout,
                                    batch_first=True)
        self.answer_encoder_lstm = nn.LSTM(self.args.embedding_size,
                                    self.args.encoder_hidden_size,
                                    num_layers=self.args.num_layers,
                                    bidirectional=True,
                                    dropout=self.args.dropout,
                                    batch_first=True)
        self.decoder_lstm_cell = nn.LSTMCell(self.args.embedding_size + self.args.decoder_hidden_size + self.args.encoder_hidden_size*2,
    ...
    def sentence_encode(self, src_input, src_input_lengths, graph_tag_input):
        ...
        input_embed = torch.cat([src_input_embed, graph_tag_input_embed], dim=-1)
        ...
        return encoder_outputs, (h_t, c_t)

    def answer_encode(self, src_input, src_input_lengths):
        sorted_src_input_length, idx_sort = torch.sort(src_input_lengths, dim=0, descending=True)
        _, idx_unsort = torch.sort(idx_sort, dim=0, descending=False)
        src_input = src_input[idx_sort]
        ...
        packed_input_embed = nn.utils.rnn.pack_padded_sequence(src_input_embed, sorted_src_input_length, batch_first=True)
        encoder_outputs, (h_t, c_t) = self.answer_encoder_lstm(packed_input_embed)
        encoder_outputs, _ = nn.utils.rnn.pad_packed_sequence(encoder_outputs, batch_first=True)
    ...
    def decode(self, tgt_input, init_hidden, encoder_outputs, answer_outputs):
        ...
        for t in range(tgt_input.size(1)):
            ...
            o_state = cur_context
            for l in range(4):
                p_l = F.softmax(torch.bmm(answer_outputs, o_state.unsqueeze(-1)), 1)
                o_state = torch.sum(torch.mul(p_l, answer_outputs), 1)

            input_embed = self.dropout(torch.cat([self.tgt_embedding(cur_input), cur_context, o_state], -1))
    ...
    def test_decoder(self, cur_input, init_hidden, t):
        if cur_input.size(0) == self.args.batch_size:
            ...
            answer_outputs = torch.cat([self.answer_outputs.unsqueeze(1),]*self.args.beam_size, 1).view(-1, self.answer_outputs.size(1), self.answer_outputs.size(2))
        ...
        if t == 0:
            ...
            cur_context = self.sentence_attn(init_hidden[0], encoder_outputs)[1].squeeze(1)
        ...
        # concat_output = h_t.unsqueeze(1)
        _, cur_context_1 = self.sentence_attn(h_t, encoder_outputs)
        concat_output = torch.cat([h_t.unsqueeze(1), cur_context_1], -1)

        tanh_fc = F.relu(self.dense(concat_output))
        logit = self.out(tanh_fc).squeeze(1)

        prob = F.softmax(logit, -1)
        return prob, init_hidden

    def forward(self, src_input, src_input_lengths, graph_tag_input, answer_input, answer_input_lengths, tgt_input):
        encode_outputs, (_, _) = self.sentence_encode(src_input, src_input_lengths, graph_tag_input)

        answer_outputs, (h_t, c_t) = self.answer_encode(answer_input, answer_input_lengths)
        init_h = torch.cat((h_t[0], h_t[1]), dim=-1)
        init_c = torch.cat((c_t[0], c_t[1]), dim=-1)
        init_state = (init_h, init_c)

        if not self.is_test:
            logits = self.decode(tgt_input, init_state, encode_outputs, answer_outputs)
            return logits
        else:
