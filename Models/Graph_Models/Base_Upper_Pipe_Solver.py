# Solver for baseline model, upper bound model and pipeline model
...
# from Compared_Models.Baseline_Model import Generator   # for baseline model
from Compared_Models.UpperBound_Model import Generator # for upper bound and pipiline model
import sys
sys.path.append('..')
...
wh_words = ['what', 'which', 'when', 'where', 'who', 'whom', 'whose', 'why', 'how']
...
class Generator_Solver(object):
    ...
    def train(self):
        ...
        for epoch in range(self.args.num_epoches):
            ...
            target_decode_batches, input_answer_batches, _ = get_graph_train_data_each_epoch(params, is_shuffle=True)
            ...
            for batch in range(n_chunk):
                ...
                logits = model(src_input, answer_tagging_input, graph_tagging_input, distance_tagging_input, src_input_length, node_input, edge_input, adj_input, answer_input, tgt_input)
...
class Test(object):
        ...
        for index in range(generate_questions_length):
            if long_result:
                if index in self.long_data_indexes: # long data
                    ...
                    reference_questions = val_questions[index]
    ...
    def evaluate(self, model=None):
        if model is None:
            print(self.args.QG_model_file)
            old_model = torch.load(self.args.QG_model_file)
            model = Generator(self.args, self.word_to_inx_src['PAD'], self.src_embeddings, self.tgt_embeddings)
            print(model.tgt_embedding.weight.size())
        ...
        for batch in range(n_chunk):
            ...
            if self.args.beam_size > 1:
                # beam search
                pre_index = model(src_input, answer_tagging_input, graph_tagging_input, distance_tagging_input, src_input_length, node_input, edge_input, adj_input, answer_input, tgt_input)
            else:
                logits = model(src_input, answer_tagging_input, graph_tagging_input, distance_tagging_input,
                               src_input_length, node_input, edge_input, adj_input, answer_input, tgt_input)
